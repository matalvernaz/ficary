...
def _emit(progress: Callable[[str], None] | None, line: str) -> None:
    ...
    progress) is invisible to a postmortem of ``ficary.log``. Routing
...
def _prepare_chapter_html_with_llm_fallback(
    ...
    * :class:`~ficary.attribution.LLMUnavailable` (non-timeout) —
      ...
      ``llm_disabled=True``.
    * :class:`~ficary.attribution.LLMTimeout` — endpoint accepted
      the connection but the model didn't reply in
      :func:`~ficary.attribution._llm_request_timeout_s` seconds.
...
def _llm_an_save_cache(path: Path | None, data: dict) -> None:
    ...
    :func:`~ficary.atomic.atomic_write_text` guarantees old-or-new on
    disk, never partial.
    """
    if path is None:
        return
...
def export_epub(
    ...
    try:
        ...
        raise ImportError(
            ...
            "Install it with: pip install 'ficary[epub]'  (or pip install ebooklib)"
        )
    ...
    if fmt == "epub":
        ...
        try:
            ...
            raise ImportError(
                ...
                "Install it with: pip install 'ficary[audio]'  (or pip install edge-tts)"
            )
