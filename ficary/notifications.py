...
* **Email** — re-uses the SMTP config from :mod:`ficary.mailer` that
...
# User-Agent identifies ficary to the remote side for diagnostics.
USER_AGENT = f"ficary/{__version__} (+watchlist)"
...
def dispatch(
    ...
    ``prefs`` is the :class:`ficary.prefs.Prefs` instance; credential
