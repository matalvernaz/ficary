...
def _root() -> Path:
    ...
    pip-installed ficary (or a dev checkout) we keep ``~/.ficary/neural``.
...
BOOTSTRAP_DONE = PY_DIR / ".ficary-bootstrap-ok"  # sentinel — only written on full success
...
def is_supported() -> bool:
    ...
    ficary already has a real Python interpreter it can reuse via
...
def activate() -> None:
    ...
    Called at package import time from ``ficary/__init__.py``. Safe
    ...
    frozen ficary.exe bundle (it only bundles stdlib modules it can
    statically detect as imported — e.g. BookNLP's transitive deps
    import ``timeit`` but ficary doesn't) falls back to the full
...
def ensure_embed_python(log_callback=None) -> bool:
    ...
    Idempotent — reads a ``.ficary-bootstrap-ok`` sentinel so the
    30-second setup runs only once per machine. Returns True when
    ``python_exe()`` is ready to run ``-m pip``.
    """
    if (BOOTSTRAP_DONE.exists() or (PY_DIR / ".ffn-dl-bootstrap-ok").exists()) and python_exe().exists():
